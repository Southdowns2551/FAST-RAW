/**
 * Rework Out submission route.
 * POST /api/rework-out - insert a new Rework Out record with optional load images.
 * GET /api/rework-out - list recent Rework Out records.
 * POST /api/rework-out/:id/email-report - re-send report.
 *
 * Body: started_at, location_street, location_area, location_lat, location_lng,
 * recycler_name, grades_sent (array of {grade}), vehicle_registration,
...
const { sendReworkOutReport } = require('../email');

const router = express.Router();

const UPLOADS_ROOT = path.join(__dirname, '..', 'uploads', 'rework-out');

/**
 * Saves base64 JPEG images to disk and returns relative paths.
 * @param {number} submissionId - Rework Out submission ID
...
function saveLoadImages(submissionId, base64Images) {
  ...
  base64Images.forEach((b64, idx) => {
    ...
    saved.push(path.join('rework-out', String(submissionId), filename));
  });
  return saved;
}
...
function saveInvoiceImage(submissionId, base64Str) {
  ...
  return path.join('rework-out', String(submissionId), filename);
}

/**
 * POST / - create Rework Out submission.
 * @param {Object} req.body - Rework Out payload
...
router.post('/', async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(
      `INSERT INTO rework_out_submissions (
        started_at, location_street, location_area, location_lat, location_lng,
        recycler_name, grades_sent, vehicle_registration,
    ...
    if (Array.isArray(load_images) && load_images.length > 0) {
      ...
      if (savedPaths.length > 0) {
        await pool.execute(
          'UPDATE rework_out_submissions SET load_images = ? WHERE id = ?',
    ...
    if (typeof invoice_image === 'string' && invoice_image.length > 0) {
      ...
      await pool.execute(
        'UPDATE rework_out_submissions SET invoice_image = ? WHERE id = ?',
        [invoicePath, id]
      );
    }

    const [rows] = await pool.query('SELECT * FROM rework_out_submissions WHERE id = ?', [id]);
    if (rows[0]) {
      ...
      sendReworkOutReport(rows[0], absolutePaths, absInvoicePath).catch((e) => console.error('[Email] Rework Out send error:', e));
    }
    res.status(201).json({ id, message: 'Rework Out submission saved' });
  } catch (err) {
    console.error('Rework Out insert error:', err);
    res.status(500).json({ error: 'Failed to save submission' });
  }
});

/**
 * GET / - list recent Rework Out submissions.
 * @returns {Object[]} rows
 */
router.get('/', async (_req, res) => {
  try {
    const [rows] = await pool.query(
      'SELECT * FROM rework_out_submissions ORDER BY created_at DESC LIMIT 100'
    );
    res.json(rows);
  } catch (err) {
    console.error('Rework Out list error:', err);
    res.status(500).json({ error: 'Failed to fetch submissions' });
  }
});

/**
 * POST /:id/email-report - re-send Rework Out report.
...
router.post('/:id/email-report', async (req, res) => {
  try {
    ...
    const invoiceImgPath = rows[0].invoice_image
      ...
      : null;
    await sendReworkOutReport(rows[0], imgPaths, invoiceImgPath);
    res.json({ message: 'Report sent' });
  } catch (err) {
    console.error('Rework Out email report error:', err);
