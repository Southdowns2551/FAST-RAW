...
| /api/portal/submissions | GET | Paginated, filterable listing across all 5 submission tables; query params: `type`, `from`, `to`, `supplier`, `grade`, `department` (waste only, exact match on the reporting department — `Lumps` selects Extrusion rows with `waste_type = 'Lumps'`, `Extrusion` excludes them), `page`, `limit`. When waste is in scope the response also carries `waste_totals: { total_kg, by_department: [{ department, total_kg, count }] }` summed over the whole filtered period, with `Lumps` as its own bucket |
