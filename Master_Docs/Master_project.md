...
| /api/waste | POST | Submit Waste record (uploads report ZIP to Egnyte); body `{ waste_date, shift, department, waste_type, kg }`. `waste_type` is required for Extrusion and must be `Waste`, `Lumps` or `Trimmings`; it must be omitted (stored `NULL`) for every other department |
...
| /api/portal/submissions | GET | Paginated, filterable listing across all 5 submission tables; query params: `type`, `from`, `to`, `supplier`, `grade`, `department` (waste only, exact match on the reporting department — `Lumps` and `Trimmings` select the Extrusion rows carrying that `waste_type`, `Extrusion` excludes both), `page`, `limit`. When waste is in scope the response also carries `waste_totals: { total_kg, by_department: [{ department, total_kg, count }] }` summed over the whole filtered period, with `Lumps` and `Trimmings` as their own buckets |
...
| 2026-08-12 | Portal print report: a "Print / Save as PDF" button appears once a search returns rows. It walks the API pages at `limit=100` (capped at 20 pages / 2000 rows, with an on-report notice when truncated) so the printout covers every match rather than just the visible page, then renders a compact table into `#portal-print-report` and calls `window.print()` — the browser's own "Save as PDF" destination covers the PDF case. Deliberately client-side: server-side rendering would mean putting the headless Chromium removed earlier back into the image. Waste-only searches get a waste column set (Date, Shift, Department, Kg, Completed by) with the period total repeated in the header and a total row in the table footer; other searches get the general set. The print container is a direct child of `body`, so `@media print` hides everything else with a single `body > *:not(#portal-print-report)` rule. Filter query building was extracted into `buildFilterParams()` so the listing and the report cannot drift apart. Note `window.print()` may be a no-op inside an installed iOS PWA; Safari tabs and desktop browsers are unaffected. SW cache v55. |
| 2026-08-12 | Static asset cache headers fixed: `nginx.conf` only set no-cache on `sw.js`, `index.html` and `styles.css`, so every other file — `portal.js`, `app.js`, `waste.js` and the rest — fell through to `location /` and was served with **no** `Cache-Control` header at all. Browsers then applied heuristic freshness (roughly 10% of the time since `Last-Modified`) and reused old copies for hours, silently defeating every JavaScript deployment. The symptom that exposed it: the new Print / Save as PDF button rendered (fresh `index.html`) and the print styles applied (fresh `styles.css`), but clicking did nothing at all because the stale `portal.js` never attached the listener. Replaced the two exact-match blocks with one `location ~* \.(?:html\|css\|js\|json)$ { expires -1; }` regex block, mirrored into `nginx-ssl.conf`. `expires` is used rather than `add_header` because a location containing any `add_header` discards inherited ones — the old `index.html` block was silently dropping `X-Frame-Options` and `X-Content-Type-Options`, which now survive. `no-cache` means revalidate, not refetch, so unchanged files still answer `304` via the existing ETag. `location = /sw.js` is left intact (an exact match outranks a regex). Hardened `sw.js` as well: the network-first handler now fetches assets through `new Request(req, { cache: 'no-store' })` so the service worker bypasses the HTTP cache entirely and a future header regression cannot pin clients to old code; navigations keep the original request since a navigate-mode Request cannot be reconstructed. SW cache v56. |
| 2026-08-12 | Waste: added a fifth department, `Off-Cuts`, appended after Bagging in both the Waste form and the Portal department filter (`index.html`) and in the `DEPARTMENTS` whitelist in `server/routes/waste.js`, which rejects unknown departments with a 400. It behaves like Printing/Slitting/Bagging — no Waste/Lumps Type dropdown, since `WASTE_TYPE_DEPARTMENT` remains `Extrusion`. No migration needed: `waste_submissions.department` is `VARCHAR(50)` with no enum or check constraint. SW cache v57. |
| 2026-08-31 | Portal waste reporting: Extrusion lumps are now reported as their own department so lump tonnage can be tracked separately. A single SQL expression, `WASTE_DEPT_LABEL` in `server/routes/portal.js` (`CASE WHEN department = 'Extrusion' AND waste_type = 'Lumps' THEN 'Lumps' ELSE department END`), is shared by the department filter in `buildWhereClause()`, the `entity_name` column in `entityCol('waste')` and the `GROUP BY` in `fetchWasteTotals()`, so the filter, the row labels and the totals cannot drift apart. Consequences: the summary chips and the printed report now show `Extrusion` (non-lump waste only) and `Lumps` as separate buckets — the grand total is unchanged — the Department column of the listing/print rows reads `Lumps` for those rows, filtering `Department: Lumps` returns only lump rows and `Department: Extrusion` deliberately excludes them. New `Lumps` option in the portal `#portal-department` filter (`index.html`), placed after Extrusion. No schema, migration or Waste-form change: `waste_type` was already captured per Extrusion submission, and the per-record report and portal detail view still show `Department: Extrusion` plus `Type: Lumps`, so a lump record stays traceable to Extrusion. `fetchWasteTotals()` sorts with `ORDER BY 1` rather than repeating the expression, because MySQL resolves ORDER BY identifiers against select aliases before table columns and the result is aliased `department`. Deployed to sab005 (both images rebuilt, containers recreated) and verified against production data: Extrusion 6 438,60 kg (61 rows) plus Lumps 191,50 kg (20 rows) equal the 6 630,10 kg the combined Extrusion bucket previously reported, with the grand total unchanged at 13 734,10 kg over 252 rows. SW cache v58. |
| 2026-09-03 | Waste: added a third Extrusion type, `Trimmings`, alongside Waste and Lumps — appended to the `#waste-type` dropdown in the Waste form (`index.html`) and to the `WASTE_TYPES` whitelist in `server/routes/waste.js`, which rejects unknown types for Extrusion with a 400. Reported separately like Lumps: `WASTE_DEPT_LABEL` in `server/routes/portal.js` was generalised from one hard-coded type to a set, `CASE WHEN department = 'Extrusion' AND waste_type IN ('Lumps', 'Trimmings') THEN waste_type ELSE department END`, so the label falls through to the type name and a future type only needs adding to the `IN` list. The Extrusion bucket therefore covers `waste_type = 'Waste'` only, and `Trimmings` joins the summary chips, the printed report and the portal `#portal-department` filter. Type-specific copy that would go stale with each new type was generalised: the client alert now reads "Please select a type." and the API error "A valid waste type must be selected for Extrusion". No migration needed: `waste_submissions.waste_type` is `VARCHAR(20)` with no enum or check constraint. Verified against production before deploying that the generalised expression reproduces the existing buckets exactly (no Trimmings rows yet; grand total 16 395,70 kg over 295 rows). Deployed to sab005, both images rebuilt and containers recreated. SW cache v59. |
